...
public class HibernateConfig {
	...
	// 스프링 인액션 - 372P
	// setPackagesToScan 메소드는 패키지들을 검사하여 퍼시스턴스 어노테이션이 적용된 도메인 클래스를 찾는다.(다수의 도메인 클래스가 존재)
	// 여기에는 JPA의 @Entity나 @MappedSuperclass 그리고 하이버네이트의 자체적인 @Entity 어노테이션이 적용된 클래스가 포함된다.
	
	// 소수의 도메인 클래스가 존재할 때는 다음처럼 지정할 수도 있다. 
	// sessionFactory.setAnnotatedClasses(new Class<?>[]{User.class, Post.class, Reply.class});
